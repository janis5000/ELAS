...
export default function ProfileEditPage() {
  ...
  useEffect(() => {
    ...
    Backend.get("/studentconnector/profile", authConfig).then((response) => {
      ...
      if (studyProgramsRes !== "" && studyProgramsRes !== null) {
        let filteredDegree = studyProgramsRes.filter(
          (x) => x.id === profileRes.degree_id
        );
        if (filteredDegree !== []) {
          let tempDegree = studyProgramsRes.filter(
            (x) => x.id === profileRes.degree_id
          )[0];
      ...
      Backend.get(
        "/studentconnector/profile/" + profileRes.id + "/courses"
      ).then((response) => {
  ...
  const getProfileInfoAndSetProfileInfoState = () => {
    Backend.get("/studentconnector/profile/" + params.id).then((response) => {
      ...
      setInitials(
        currentProfileRes.firstname.substring(0, 1).toUpperCase() +
          currentProfileRes.lastname.substring(0, 1).toUpperCase()
      );
    });
  };
  ...
  const setNewProfileSkillsAfterPostRequest = () => {
    setCurrentProfile((prevState) => {
      ...
      if (newSkills !== "" && skillsRemove.length > 0) {
        ...
        newSkillsList = newSkillsList.filter(
          (x) => !prevProfile.skills.includes(x)
        );
        prevProfile.skills.push(...newSkillsList);
        skillsRemove.forEach(
          (x) =>
            (prevProfile.skills = prevProfile.skills.filter((e) => e !== x))
        );
